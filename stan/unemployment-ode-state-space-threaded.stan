// Unemployment ODE State Space Model - Threaded Version (CURRENTLY NON-FUNCTIONAL)
//
// ⚠️  KNOWN LIMITATION - DOES NOT RUN ⚠️
//
// This file implements multi-threading via reduce_sum() but CANNOT BE USED due to
// fundamental Stan type constraints. The model compiles successfully but crashes at
// runtime when Stan attempts to pass incompatible types to reduce_sum().
//
// WHY IT DOESN'T WORK:
// - Stan's reduce_sum() only accepts: vector, matrix, array[] real, array[] int (1D)
// - Our model requires: array[T] vector[N_edu] u (state trajectories)
//                       array[T, N_edu] int (observation counts)
// - These types are essential to the ODE state-space formulation
//
// WHAT'S IMPLEMENTED (for future reference):
// - Complete reduce_sum() infrastructure with partial_likelihood() function
// - Grainsize parameter for tuning parallelization granularity
// - All TDD tests pass for structure (runtime tests reveal the limitation)
//
// CURRENT STATUS:
// This code remains as documentation of best practices and will become immediately
// usable if future Stan versions add support for array[] vector and multi-dimensional
// array types in reduce_sum().
//
// USE INSTEAD: unemployment-ode-state-space-efficient.stan (serial version, 28 min runtime)
//
// Based on efficient version with:
// - SPLINE BASIS for latent deviations (K_spline << T parameters)
// - ODE dynamics + shocks + seasonality for structural variation
// - Non-centered parameterization for sampling efficiency
//
// Author: Claude Code (TDD implementation)
// Date: 2026-01-21
// Reference: docs/PROFILING_GUIDE.md - Known Limitations section
...
functions {
  ...
  // Partial likelihood function for reduce_sum parallelization
  // Computes log likelihood for observations from obs_slice[start] to obs_slice[end]
  real partial_likelihood(
    array[] int obs_slice,  // Slice of observation indices to process
    int start,              // Start index in slice (inclusive)
    int end,                // End index in slice (inclusive)
    // Predicted unemployment rates from ODE
    array[] vector u,
    // Observation data
    array[,] int n_unemployed,
    array[,] int n_total,
    // Model parameters
    real phi,
    // Dimensions
    ...
    for (idx in start:end) {
      int obs_idx = obs_slice[idx];
      int t = ((obs_idx - 1) %/% N_edu) + 1;
      int i = ((obs_idx - 1) % N_edu) + 1;

      if (n_total[t, i] > 0) {
        real u_safe = fmin(fmax(u[t][i], 1e-6), 1 - 1e-6);
        real alpha = u_safe * phi;
        real beta_param = (1 - u_safe) * phi;
        lp += beta_binomial_lpmf(n_unemployed[t, i] | n_total[t, i], alpha, beta_param);
      }
    }
    return lp;
  }
...
data {
  ...
  // Threading control
  int<lower=1> grainsize;  // Observations per thread (1 = fine-grained)
}
...
transformed data {
  ...
  // Create flat array of observation indices for reduce_sum
  int N_obs = T * N_edu;
  array[N_obs] int obs_indices;
  for (obs_idx in 1:N_obs) {
    obs_indices[obs_idx] = obs_idx;
  }
}
...
model {
  ...
  // === Likelihood (parallelized with reduce_sum) ===

  target += reduce_sum(
    partial_likelihood,
    obs_indices,      // Array of observation indices to split
    grainsize,        // Observations per thread chunk
    u, n_unemployed, n_total, phi, T, N_edu
  );
}
