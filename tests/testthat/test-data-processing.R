...
test_that("calculate_monthly_unemployment aggregates by month correctly", {
  source(here::here("R", "data-processing.R"))
  ...
  test_data <- data.frame(
    ...
    MONTH = c(1, 1, 1, 2, 2, 2),
    EMPSTAT = c(10, 10, 20, 10, 20, 20),
    WTFINL = c(1000, 1000, 1000, 1000, 1000, 1000)
  )
  ...
  # Should have YEAR, MONTH, unemployment_rate, n_obs columns
  expect_true(all(c("YEAR", "MONTH", "unemployment_rate", "n_obs") %in% names(result)))

  # January: 1/3 unemployed
  jan <- result[result$MONTH == 1, ]
  expect_equal(jan$unemployment_rate, 1/3, tolerance = 0.01)
  expect_equal(jan$n_obs, 3)

  # February: 2/3 unemployed
  feb <- result[result$MONTH == 2, ]
  expect_equal(feb$unemployment_rate, 2/3, tolerance = 0.01)
  expect_equal(feb$n_obs, 3)
})

test_that("calculate_monthly_unemployment preserves YEAR information", {
  source(here::here("R", "data-processing.R"))
  ...
  # Should have 2 rows (2023-12 and 2024-01)
  expect_equal(nrow(result), 2)

  # Should have correct years
  expect_true(2023 %in% result$YEAR)
  expect_true(2024 %in% result$YEAR)
})

test_that("calculate_monthly_unemployment sorts chronologically", {
  source(here::here("R", "data-processing.R"))
  ...
  # Should be sorted: 2023-12, 2024-01, 2024-02
  expect_equal(result$YEAR[1], 2023)
  expect_equal(result$MONTH[1], 12)
  expect_equal(result$YEAR[nrow(result)], 2024)
  expect_equal(result$MONTH[nrow(result)], 2)
})
