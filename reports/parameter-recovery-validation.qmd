...
ggplot(coverage_df, aes(x = coverage)) +
  ...
  # Target line
  geom_vline(xintercept = 95, color = "red", linetype = "dashed", linewidth = 1.2) +
  # Observed mean line
  geom_vline(xintercept = mean_coverage * 100, color = "darkgreen", linewidth = 1.2) +
  # Text annotations for clarity
  annotate("text", x = 95, y = Inf, label = sprintf("Target: 95%%"),
           vjust = 1.5, hjust = -0.1, color = "red", size = 4, fontface = "bold") +
  annotate("text", x = mean_coverage * 100, y = Inf,
           label = sprintf("Observed Mean: %.1f%%", mean_coverage * 100),
           vjust = 1.5, hjust = ifelse(mean_coverage * 100 > 96, 1.1, -0.1),
           color = "darkgreen", size = 4, fontface = "bold") +
  # Add explanatory annotation box
  annotate("rect", xmin = 85.5, xmax = 93, ymin = 28, ymax = 34,
           fill = "white", color = "gray50", alpha = 0.95, linewidth = 0.5) +
  annotate("text", x = 89.25, y = 32.5,
           label = "Well-Calibrated Model:",
           size = 3, fontface = "bold", hjust = 0.5) +
  annotate("text", x = 89.25, y = 31,
           label = "Each simulation generates new data",
           size = 2.5, hjust = 0.5) +
  annotate("text", x = 89.25, y = 29.8,
           label = "from the true model. We count what %",
           size = 2.5, hjust = 0.5) +
  annotate("text", x = 89.25, y = 28.6,
           label = "of points fall in the 95% PI. Distribution",
           size = 2.5, hjust = 0.5) +
  scale_x_continuous(limits = c(85, 100), breaks = seq(85, 100, by = 5)) +
  labs(
    title = "Prediction Interval Calibration Test",
    subtitle = sprintf("%d simulations × 120 data points each: Does 95%% PI capture ~95%% of new data?", n_sims),
    x = "% of Data Points Within 95% Prediction Interval (per simulation)",
    y = "Count",
    caption = "Distribution should center near 95% for well-calibrated intervals"
  ) +
  theme(
...
for (i in 1:n_show) {
  ...
  preds <- predict(model, newdata = pred_grid, se.fit = TRUE)
  baseline_est <- mean(preds$fit)
  baseline_se <- sqrt(sum(preds$se.fit^2)) / 12
...
ggplot(ci_data, aes(x = sim, y = est, color = covered)) +
  ...
  annotate("rect", xmin = 1, xmax = 15, ymin = 0.0258, ymax = 0.0262,
           fill = "white", color = "gray50", alpha = 0.9, linewidth = 0.5) +
  annotate("text", x = 8, y = 0.02605,
           label = sprintf("Coverage Summary (n=%d):", n_show),
           size = 3.5, fontface = "bold") +
  annotate("text", x = 8, y = 0.026,
           label = sprintf("Contains: %d (%.0f%%)", n_contains, pct_contains),
           size = 3, color = "steelblue") +
  annotate("text", x = 8, y = 0.02595,
           label = sprintf("Misses: %d (%.0f%%)", n_misses, pct_misses),
           size = 3, color = "orange") +
  scale_color_manual(
