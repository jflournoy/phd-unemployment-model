...
- **The Test:** The `validate_parameter_recovery_coverage()` function:
  1. Takes the reference model and extracts its estimated parameters
  2. Generates 100 NEW datasets using the DGP parameters specified in `true_params`
  3. For each new dataset, fits a NEW GAM and checks:
     - Does the reference model's prediction interval contain new observations?
     - Does the new model's baseline CI contain the true baseline?
     - Does the new model's amplitude CI contain the true amplitude?
  4. Computes coverage rates across all 100 simulations
