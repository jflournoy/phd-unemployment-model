...
**Equilibrium trajectory** (`u_eq[t]`): The time-varying structural unemployment rate
toward which the ODE pulls. Constructed from a B-spline basis with a random walk prior
for smoothness, this replaces the previous constant-equilibrium + separate-spline-deviation
architecture.
...
> **Note:** In the current model (v4), there is no separate "spline component." The
> B-spline constructs the time-varying equilibrium $u_{\text{eq}}(t)$ directly. The
> residual plot above was generated from the previous model architecture and is kept
> for reference. Updated figures will replace this in a future render.
