...
validate_parallel_model_exists(
  serial_file = here("models", "ode-state-space-efficient-fit.qs")
)
...
1. **Most Variable**: `r variance_summary[1, parameter]` (σ = `r sprintf("%.2f", variance_summary[1, sigma_mean])`)
...
2. **Least Variable**: `r variance_summary[.N, parameter]` (σ = `r sprintf("%.3f", variance_summary[.N, sigma_mean])`)
...
3. **Practical Implications**:
   - Parameters with weak pooling (high σ) require education-specific interpretation
   - Parameters with tight pooling (low σ) can be summarized at the population level
...
# Load model from targets pipeline (required) with fallback to RDS cache
if (file.exists(targets_cache)) {
  ...
  cat("WARNING: Using legacy RDS cache instead of targets pipeline QS format.\n")
  cat("Recommend running: targets::tar_make(model_ode_state_space_efficient)\n\n")
...
- Most β parameters are not significantly different from zero (90% credible intervals include zero), except for equilibrium unemployment (β = `r round(beta_summary$mean[beta_summary$parameter == "Logit u eq"], 3)`, 90% CI [`r round(beta_summary$q5[beta_summary$parameter == "Logit u eq"], 3)`, `r round(beta_summary$q95[beta_summary$parameter == "Logit u eq"], 3)`] excludes zero).
...
- The LOO-CV comparison indicates that adding education trends does not improve overall predictive accuracy (ELPD difference = `r round(elpd_diff, 2)` ± `r round(se_diff, 2)`, not statistically significant), suggesting that the hierarchical structure already captures education-specific heterogeneity adequately.
...
serial_file <- here("models", "ode-state-space-efficient-fit.qs")
parallel_file <- here("models", "ode-state-space-edu-parallel-fit.qs")
...
if (file.exists(serial_file) && file.exists(parallel_file)) {
  ...
  validate_parallel_model_exists(parallel_file, serial_file)
}
```
