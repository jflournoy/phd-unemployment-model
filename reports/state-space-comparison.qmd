...
#### 2.3 Seasonal Effects (Two Components)

The model uses **two seasonal components** to capture observed patterns:

**1. Finding Rate Seasonality**: Job finding rates vary seasonally (academic hiring, summer jobs):

$$f_i^{\text{eff}}(t) = f_i \cdot (1 + \gamma^f_{m(t),i})$$

This affects the rate at which unemployed workers find jobs. Prior: $\gamma^f_{m,i} \sim \text{Normal}(0, 0.10)$.

**2. Direct Unemployment Seasonality**: A direct seasonal effect on the unemployment rate (logit scale):

$$\text{logit}(u_{t,i}) = \text{logit}(u_{t-1,i}) + \Delta u_{t,i} + \gamma^u_{m(t),i} + \epsilon_{t,i}$$

This captures observed seasonal patterns that may not be fully explained by finding rate dynamics (e.g., academic calendar effects for PhDs). Prior: $\gamma^u_{m,i} \sim \text{Normal}(0, 0.05)$.

**Why two components?** The finding rate effect is economically interpretable (hiring seasonality), but its impact on unemployment is dampened by the ODE integration. The direct effect captures any additional observed seasonality.
...
### Monthly Seasonal Pattern: Direct Effect on Unemployment

The model now includes a **direct seasonal effect on unemployment** (on the logit scale), in addition to the indirect effect through finding rates. This captures observed seasonal patterns like academic hiring calendars.
...
### Monthly Seasonal Pattern: Finding Rate

The effect of seasonality on job finding rates (how easy it is to find a job):

```{r monthly-seasonal-finding}
#| fig-cap: "Monthly seasonal effects on job finding rate by education level"
#| fig-height: 8

# Extract finding rate seasonal parameters
seasonal_f_summary <- result$fit$summary(variables = "seasonal_finding")

# Parse indices
seasonal_f_summary$month <- as.integer(
  gsub("seasonal_finding\\[(\\d+),\\d+\\]", "\\1", seasonal_f_summary$variable)
)
seasonal_f_summary$edu_index <- as.integer(
  gsub("seasonal_finding\\[\\d+,(\\d+)\\]", "\\1", seasonal_f_summary$variable)
)
seasonal_f_summary$education <- stan_data$education_levels[seasonal_f_summary$edu_index]

# Create month labels
seasonal_f_summary$month_label <- factor(month_labels[seasonal_f_summary$month],
                                          levels = month_labels)

# Plot for selected education levels
seasonal_f_plot <- seasonal_f_summary[seasonal_f_summary$education %in% sample_edu, ]

ggplot(seasonal_f_plot, aes(x = month_label, y = mean * 100, group = education)) +
  geom_ribbon(aes(ymin = q5 * 100, ymax = q95 * 100, fill = education),
              alpha = 0.3) +
  geom_line(aes(color = education), linewidth = 1.2) +
  geom_point(aes(color = education), size = 3) +
  ...
  facet_wrap(~education, ncol = 1, scales = "free_y") +
  labs(x = "Month",
       y = "Seasonal Effect on Finding Rate (%)",
       title = "Seasonal Effects on Job Finding Rate",
       subtitle = "Positive = easier to find jobs; Negative = harder; Band = 90% CI") +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1),
        strip.text = element_text(size = 12, face = "bold"))
```
...
ggplot() +
  ...
  scale_color_manual(values = c("Pure ODE" = "#E74C3C",
                                "Trend (no seasonal)" = "#F39C12",
                                "Full Model" = "#3498DB")) +
  scale_x_continuous(breaks = seq(floor(max_year - 1), ceiling(max_year), 0.25),
