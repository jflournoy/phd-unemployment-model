...
# Check for edu-parallel model (preferred) or monotonic spline model
edu_par_file <- here("models", "ode-state-space-edu-parallel-fit.rds")
ms_file <- here("models", "ode-state-space-monotonic-spline-fit.qs")
ms_file_rds <- here("models", "ode-state-space-monotonic-spline-fit.rds")

if (!file.exists(edu_par_file) && !file.exists(ms_file) && !file.exists(ms_file_rds)) {
  stop(
    "No fitted model found. Expected one of:\n",
    "  ", edu_par_file, "\n",
    "  ", ms_file, "\n",
    "  ", ms_file_rds, "\n",
    "Run the targets pipeline or fit the model manually.\n",
    call. = FALSE
  )
}
...
possible_model_files <- c(
  ...
  here("models", "ode-state-space-monotonic-spline-fit.qs"),
  here("models", "ode-state-space-monotonic-spline-fit.rds"),
  here("models", "ode_hierarchical_v2_fit.rds")
)
...
**Spline component** (`u_trend - u_ode_pure`): The part of the de-seasonalized trend
that the B-spline contributes beyond the ODE dynamics. This is what the spline
is "explaining" that the equilibrium + shock dynamics cannot.

```{r fig-residual-spline, fig.cap="Spline component: what the B-spline adds beyond ODE dynamics"}
knitr::include_graphics("figures/residual-spline-component.png")
```

> **Note:** The large positive spline jump for "less than high school" in Spring 2026
> is a boundary artifact. The B-spline's finite basis support becomes unreliable near
> the end of the time series. The observed rate for this group actually *fell* in
> April 2026 (8.74% → 8.29%), but the spline bends up sharply (+2.5 pp) due to
> reduced basis function overlap at the boundary. This does not affect the
> economic parameters or other education groups meaningfully.
...
key_params <- c(
  ...
  # Hierarchical spline smoothness (per-education)
