...
cached_model_path <- here("models", "ode-state-space-fit.qs")
...
if (use_cached) {
  ...
  result <- qs::qread(cached_model_path)
} else {
  ...
  qs::qsave(result, cached_model_path)
...
if (n_divergent > 0) {
  ...
  p_pairs <- mcmc_pairs(
    draws,
    pars = c("mu_separation", "sigma_separation", "separation_rate[1]", "sigma_state"),
    np = np,
...
p1 <- mcmc_trace(draws, pars = "mu_separation", np = np) +
  labs(title = "Hierarchical Mean: Separation Rate")

p2 <- mcmc_trace(draws, pars = "sigma_separation", np = np) +
  labs(title = "Hierarchical SD: Separation Rate")
...
key_params <- c("mu_separation", "sigma_separation", "mu_finding", "sigma_finding",
