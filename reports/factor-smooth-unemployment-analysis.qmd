...
subtitle: "Joint Modeling of Education-Specific Unemployment Trends, 2000-2025"
...
# NOTE: This report currently uses simulated data for demonstration
# In production, replace with actual CPS data file

# For this demo, create simulated multi-education data
# Using boundary-safe parameters (validated in tests)
set.seed(42)
demo_data <- simulate_multi_education_unemployment(
  n_years = 11,  # 2000-2010 (boundary-safe for 11 years)
  education_levels = c("phd", "masters", "bachelors"),
...
# Add year column
demo_data$year <- 2000 + (demo_data$time_index - 1) %/% 12

# Save to temporary file for analysis functions
temp_data_file <- tempfile(fileext = ".rds")
saveRDS(demo_data, temp_data_file)

cat("Data Summary:\n")
cat("  Years:", min(demo_data$year), "-", max(demo_data$year), "\n")
cat("  Observations:", nrow(demo_data), "\n")
cat("  Education levels:", paste(unique(demo_data$education), collapse = ", "), "\n")
```

::: {.callout-note}
## Production Data

In production use, replace the simulated data above with:
```r
data_file <- here("data", "cps_multi_education_unemployment.rds")
```
:::
