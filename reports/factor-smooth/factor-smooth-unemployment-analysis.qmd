...
# Filter for 2024-2025 period
recent_data <- actual_data[actual_data$year >= 2024, ]

# Get unique time points with their ACTUAL time_index values from the fitted model
# This is critical - we need the same time_index scale the model was trained on
time_periods <- unique(recent_data[, c("year", "month", "time_index")])
...
# CRITICAL: time_index uses ACTUAL values from data (for s(time_index) trend)
#           month is FIXED to 6 (for s(month) seasonal component only)
pred_grid <- expand.grid(
  time_index = unique(time_periods$time_index),  # Use actual time_index values
  month = 6,  # Fixed month to remove seasonal variation
  education = factor(education_levels, levels = education_levels)
)

# Add date information for plotting (each time_index maps to a specific year-month)
pred_grid <- merge(pred_grid, time_periods[, c("time_index", "date")], by = "time_index")
