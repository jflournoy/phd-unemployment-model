...
# Get actual data to extract correct time_index values
actual_data <- analysis$data

# Filter for 2024-2025 period
recent_data <- actual_data[actual_data$year >= 2024, ]

# Get unique time points (one per month)
time_lookup <- unique(recent_data[, c("year", "month", "time_index")])
time_lookup$date <- as.Date(paste(time_lookup$year, time_lookup$month, 1, sep = "-"))

# Create grid for all education levels using actual time_index values
pred_grid <- expand.grid(
  time_index = time_lookup$time_index,
  education = factor(education_levels, levels = education_levels)
)

# Add date and fixed month (June = 6 to remove seasonality)
pred_grid <- merge(pred_grid, time_lookup[, c("time_index", "date")], by = "time_index")
