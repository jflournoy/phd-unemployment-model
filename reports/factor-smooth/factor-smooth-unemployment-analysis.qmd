...
# Calculate recent changes for each comparison
focus_start <- as.Date("2024-01-01")
recent_end <- max(viz_data$trends$date)

phd_focus <- viz_data$trends[viz_data$trends$education == "phd" &
                             viz_data$trends$date >= focus_start, ]

for (edu in setdiff(education_levels, "phd")) {
  comp_focus <- viz_data$trends[viz_data$trends$education == edu &
                                viz_data$trends$date >= focus_start, ]

  # Latest values (most recent data)
  latest_phd <- tail(phd_focus$trend_effect, 1)
  latest_comp <- tail(comp_focus$trend_effect, 1)
  latest_diff <- latest_comp - latest_phd

  # Starting values (Jan 2024)
  start_phd <- phd_focus$trend_effect[1]
  start_comp <- comp_focus$trend_effect[1]
  ...
  cat(sprintf("- Current trend difference: %.3f%% (PhD is %.3f%% %s)\n",
              abs(latest_diff) * 100,
              abs(latest_diff) * 100,
              if(latest_diff > 0) "lower" else "higher"))
  cat(sprintf("- Gap %s by %.3f%% since Jan 2024\n",
              if(abs(change_in_gap) < 0.0001) "unchanged" else if(change_in_gap > 0) "widened" else "narrowed",
              abs(change_in_gap) * 100))
  cat(sprintf("- Latest PhD trend: %.3f%%, Latest %s trend: %.3f%%\n\n",
              latest_phd * 100,
              education_labels[[edu]],
              latest_comp * 100))
}
...
### PhD vs Other Education Levels: Full Predicted Unemployment Rates

::: {.callout-important}
## Predicted Values: Complete Model Estimates

This section shows **full model predictions** (trend + seasonality + all components) comparing PhD unemployment rates to each other education level for 2024-2025.
:::

```{r phd-predicted-comparisons}
#| fig-height: 16
#| fig-cap: "Figure 3c: PhD unemployment predictions vs each education level (full model, 2024-2025)"

# Get predictions from the best model (m6)
best_model <- analysis$models$m6

# Create prediction data for 2024-2025
pred_dates <- seq(as.Date("2024-01-01"), max(analysis$data$date), by = "month")
pred_data <- expand.grid(
  date = pred_dates,
  education = factor(education_levels, levels = education_levels)
)

# Add time_index and month
pred_data$time_index <- as.numeric(pred_data$date - min(analysis$data$date)) / 30.44 + 1
pred_data$month <- as.numeric(format(pred_data$date, "%m"))

# Get predictions with SE
preds <- predict(best_model, newdata = pred_data, type = "response", se.fit = TRUE)
pred_data$predicted_rate <- preds$fit
pred_data$se <- preds$se.fit

# Extract PhD predictions
phd_preds <- pred_data[pred_data$education == "phd", ]
...
for (edu in setdiff(education_levels, "phd")) {
  ...
  comp_preds <- pred_data[pred_data$education == edu, ]
  ...
  # Add both panels to the list
  all_pred_plots[[paste0(edu, "_predicted")]] <- p1
  all_pred_plots[[paste0(edu, "_diff")]] <- p2
}

# Arrange all plots: 2 columns (predictions | difference), 6 rows
gridExtra::grid.arrange(grobs = all_pred_plots, ncol = 2, byrow = TRUE)
```

#### Predicted Rate Summary (2024-2025)

```{r phd-predicted-summary}
#| echo: false

# Calculate summary statistics for predicted rates
for (edu in setdiff(education_levels, "phd")) {
  comp_preds <- pred_data[pred_data$education == edu, ]
  phd_subset <- pred_data[pred_data$education == "phd", ]

  # Latest values
  latest_phd <- tail(phd_subset$predicted_rate, 1)
  latest_comp <- tail(comp_preds$predicted_rate, 1)
  latest_diff <- latest_comp - latest_phd

  # Average over 2024-2025
  avg_phd <- mean(phd_subset$predicted_rate)
  avg_comp <- mean(comp_preds$predicted_rate)
  avg_diff <- avg_comp - avg_phd

  cat(sprintf("**%s vs PhD (Predicted Rates):**\n", education_labels[[edu]]))
  ...
  cat(sprintf("- Current difference: %.2f%% (PhD is %.2f%% %s)\n",
              ...
              abs(latest_diff) * 100,
              if(latest_diff > 0) "lower" else "higher"))
  cat(sprintf("- 2024-2025 average PhD: %.2f%%, average %s: %.2f%%\n",
              avg_phd * 100,
              education_labels[[edu]],
              avg_comp * 100))
  cat(sprintf("- Average gap: %.2f%%\n\n",
              abs(avg_diff) * 100))
}
```

::: {.callout-note}
## Predicted Rates vs Trend-Only Comparisons

The **predicted rates** (above) include seasonal fluctuations and show the actual expected unemployment rate at each point in time.

The **trend comparisons** (previous section) show only the long-term trend component after removing seasonality, making it easier to see secular changes.

Both perspectives are valuable:
- **Predicted rates**: Answer "What is the actual unemployment rate now?"
- **Trends**: Answer "What is the underlying direction after accounting for seasonal variation?"
:::
