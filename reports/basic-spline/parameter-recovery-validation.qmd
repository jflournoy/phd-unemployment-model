...
- **Data Generating Process (DGP):** Linear trend + sinusoidal seasonality with known parameters:
  - Baseline: 0.025 (2.5%)
  - Trend slope: 0.0003 (0.03 percentage points per month)
  - Seasonal amplitude: 0.008 (0.8 percentage points)
  - Noise SD: 0.002 (0.2 percentage points)

- **Fitted Model:** GAM with flexible smooths (cubic regression spline for trend, cyclic spline for seasonality)

- **The Test:** Fit GAM to one training dataset from the DGP, then generate many NEW datasets from the SAME DGP. Do the fitted model's prediction intervals (calculated once from training data) contain the new observations at the claimed 95% rate?

**Why This Matters:** This tests whether the model's prediction intervals are honest about uncertainty when predicting truly new data from the same data-generating process.

**Method:**
1. Fit a GAM model to training data from the DGP
2. Calculate 95% prediction intervals from the fitted model
3. Generate many new datasets from the true DGP (same parameters)
...
- **Data Generating Process (DGP):** Simple model with NO trend, just baseline + seasonality:
  - True baseline: 0.025 (2.5%)
  - Trend slope: 0 (no trend to avoid contamination)
  - Seasonal amplitude: 0.006 (0.6 percentage points)
  - Noise SD: 0.002 (0.2 percentage points)

- **Fitted Model:** GAM with trend and seasonal smooths

- **The Test:** For each of 100 simulations:
  1. Generate NEW dataset from DGP with known baseline = 0.025
  2. Fit GAM to this dataset
  3. Extract baseline estimate and compute 95% CI using variance-covariance matrix
  4. Check if TRUE baseline (0.025) falls within THIS model's CI
  5. Across 100 experiments, ~95% of CIs should contain the true baseline

**Why This Matters:** This tests whether the model's confidence intervals for the baseline parameter are statistically honest - they should bracket the true value at the claimed rate across repeated experiments.
...
- **Data Generating Process (DGP):** Simple model with NO trend, just baseline + seasonality:
  - Baseline: 0.025 (2.5%)
  - Trend slope: 0 (no trend)
  - True seasonal amplitude: 0.008 (0.8 percentage points)
  - Noise SD: 0.002 (0.2 percentage points)

- **Fitted Model:** GAM with trend and seasonal smooths

- **The Test:** For each of 100 simulations:
  1. Generate NEW dataset from DGP with known amplitude = 0.008
  2. Fit GAM to this dataset
  3. Extract seasonal component and estimate amplitude as (max - min)/2
  4. Compute 95% CI for amplitude using variance-covariance matrix (accounting for correlation between max and min month estimates)
  5. Check if TRUE amplitude (0.008) falls within THIS model's CI
  6. Across 100 experiments, ~95% of CIs should contain the true amplitude

**Why This Matters:** Seasonal amplitude is a derived parameter (not directly estimated), so its CI requires careful computation of the variance of a nonlinear function. This tests whether we're correctly propagating uncertainty.
...
**Goal:** Use the `validate_parameter_recovery_coverage()` function to test all three coverage properties simultaneously in a single call.

**What We're Comparing:**

- **Reference Data DGP:** Simple model with NO trend (to match validation function defaults):
  - Baseline: 0.025 (2.5%)
  - Trend slope: 0 (NO TREND - critical for avoiding parameter mismatch!)
  - Seasonal amplitude: 0.008 (0.8 percentage points)
  - Noise SD: 0.002 (0.2 percentage points)

- **Reference Model:** GAM fitted to ONE dataset from this DGP

- **The Test:** The `validate_parameter_recovery_coverage()` function runs ALL THREE tests above:

  1. **Prediction Interval Coverage (like Test 1):**
     - Uses REFERENCE model's prediction intervals (computed once)
     - Generates 100 NEW datasets from the same DGP
     - Checks if reference model's PIs contain new observations
     - Expected: ~95% of all new observations should be within PIs

  2. **Baseline CI Coverage (like Test 2):**
     - For each of 100 simulations:
       * Generate NEW dataset from DGP
       * Fit NEW model to this dataset
       * Check if NEW model's baseline CI contains true baseline (0.025)
     - Expected: ~95% of the 100 CIs should contain true baseline

  3. **Amplitude CI Coverage (like Test 3):**
     - For each of 100 simulations:
       * Generate NEW dataset from DGP
       * Fit NEW model to this dataset
       * Check if NEW model's amplitude CI contains true amplitude (0.008)
     - Expected: ~95% of the 100 CIs should contain true amplitude

**Why This Matters:** This comprehensive test ensures all three types of uncertainty quantification (prediction intervals and two parameter CIs) are properly calibrated. It's a single-function wrapper that performs the same tests as Tests 1-3.

**CRITICAL:** The DGP used to create reference data MUST match the DGP used in the validation function. Here, both use `trend_slope = 0`. Any mismatch causes systematic bias and miscalibrated coverage.
...
### Baseline Estimation in GAMs

Due to GAM sum-to-zero constraints on smooth terms, baseline is estimated by:
1. Evaluating predictions at t=min (start of series)
2. Averaging over all 12 months to remove seasonal variation
3. This gives pure baseline without trend or seasonal contamination

### Variance Calculation for Derived Parameters

- **Baseline CI:** Uses variance-covariance matrix for mean of 12 predictions
- **Amplitude CI:** Uses variance-covariance matrix for contrast (max_month - min_month)/2
- **Prediction Intervals:** Includes both model uncertainty (SE) and residual variance

### Critical Validation Requirement

**DGP Consistency:** The data-generating process used for reference data MUST exactly match the DGP used in coverage validation simulations. Any parameter mismatch creates systematic bias and miscalibrated coverage. This is why Test 4 uses `trend_slope = 0` for both reference data and validation function.
