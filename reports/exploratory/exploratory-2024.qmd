...
1. **Average Rate**: PhD unemployment in 2024 averaged `r sprintf("%.2f%%", mean(phd_unemployment$unemployment_rate) * 100)` - significantly lower than the overall US unemployment rate (~4% in 2024)

2. **Seasonal Variation**:
   - Lowest: February (`r sprintf("%.2f%%", min(phd_unemployment$unemployment_rate) * 100)`)
   - Highest: January (`r sprintf("%.2f%%", max(phd_unemployment$unemployment_rate) * 100)`)

3. **Trend**: PhD unemployment remains consistently low throughout 2024, ranging from 0.5% to 2.0%
...
The unemployment rates shown (e.g., 1.16% mean for 2024) represent:
...
- **Labor force participants only** - excludes those not in labor force (retired, students, etc.)
...
- **Unemployed**: EMPSTAT in [20, 21, 22] (unemployed persons seeking work)
- **Not in labor force**: EMPSTAT in [30-36] (retired, school, housework, disabled, other)
- **Unemployment Rate** = Σ(weights × unemployed) / Σ(weights × labor_force)

This follows the official BLS definition of unemployment rate. Note that "not in labor force" individuals (codes 30-36) are excluded from both the numerator and denominator.
