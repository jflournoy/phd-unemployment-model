...
- **shock_2008_2009, shock_2020**: Binary indicators for major economic disruptions including recovery periods (2008-2010 financial crisis, 2020-2021 COVID-19)
- **s(time_index, by=education)**: Education-specific smooth trends (k=150) capturing long-term unemployment dynamics
- **s(time_index, by=shock_*)**: Shock-specific time dynamics (k=15, thin plate splines) allowing different crisis trajectories during extended disruption/recovery periods
...
# Add shock variables based on year (extended through recovery periods)
trend_pred_data$year <- 2000 + floor((trend_pred_data$time_index - 1) / 12)
trend_pred_data$shock_2008_2009 <- as.integer(trend_pred_data$year >= 2008 & trend_pred_data$year <= 2010)
trend_pred_data$shock_2020 <- as.integer(trend_pred_data$year >= 2020 & trend_pred_data$year <= 2021)
