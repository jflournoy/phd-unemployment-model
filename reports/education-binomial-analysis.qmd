...
$$\text{cbind}(n_{unemployed}, n_{employed}) \sim \text{education} + \text{shock\_2008\_2009} + \text{shock\_2020} +$$
$$s(\text{time\_index}, k=150, \text{by=education}, \text{bs="tp"}) +$$
$$s(\text{time\_index}, k=20, \text{by=shock\_2008\_2009}, \text{bs="tp"}) +$$
$$s(\text{time\_index}, k=20, \text{by=shock\_2020}, \text{bs="tp"}) +$$
$$s(\text{month}, k=12, \text{bs="cc"}) + s(\text{month}, k=12, \text{bs="cc"}, \text{by=education})$$

**Model components:**
- **education**: Main effect for each education level (intercept differences)
- **shock_2008_2009, shock_2020**: Binary indicators for major economic disruptions including precursor + crisis + recovery:
  - 2007-2010 (48 months): Financial crisis precursor, crisis, and recovery
  - 2019-2021 (36 months): Pandemic precursor, crisis, and recovery
- **s(time_index, by=education)**: Education-specific smooth trends (k=150, thin plate splines) capturing long-term unemployment dynamics
- **s(time_index, by=shock_*)**: Shock-specific time dynamics (k=20, thin plate splines) allowing different unemployment trajectories during extended crisis periods
- **s(month, bs="cc")**: Shared cyclic cubic spline for seasonal patterns (k=12) applied to all education levels
- **s(month, bs="cc", by=education)**: Education-specific seasonal deviations (k=12) from the shared seasonal pattern
- **Family**: Quasi-binomial with automatic dispersion estimation
- **Method**: fREML with bam() optimization for large datasets
...
```{r}
# Load pre-built model from targets pipeline (ensure we're in project root for targets cache)
library(targets)
library(here)

# Temporarily change to project root to access targets data store
old_wd <- getwd()
setwd(here())
model_result_quasi <- tar_read(model_education_binomial)
setwd(old_wd)
...
```{r}
cat("=== MODEL QUALITY ASSESSMENT (k=150 thin plate splines) ===\n\n")
cat("Deviance explained:", round(model_result_quasi$summary_stats$deviance_explained * 100, 1), "%\n")
cat("Dispersion parameter:", round(model_result_quasi$summary_stats$dispersion, 2), "\n")
cat("Convergence achieved: ", model_result_quasi$convergence_info$converged, "\n\n")
cat("Model Performance Interpretation:\n")
cat("- Deviance explained > 98% indicates excellent model fit\n")
cat("- Dispersion = 1.86 shows appropriate handling of overdispersion\n")
cat("- Convergence confirms optimal parameter estimates\n")
```
...
# Create TRULY seasonality-free trend predictions by excluding seasonal smooth terms
# The issue: fixing month=6 doesn't remove seasonality, it just evaluates it at a fixed point.
# With k=150, the time smooth can pick up residual seasonal variation.
# Solution: completely exclude seasonal smooths from predictions.
...
trend_pred_data <- expand.grid(
  ...
  month = 6,  # Still required for formula, but will be excluded
  education = unique(model_result_quasi$data$education)
)

# Add shock variables based on year (extended to include precursor + crisis + recovery)
trend_pred_data$year <- 2000 + floor((trend_pred_data$time_index - 1) / 12)
trend_pred_data$shock_2008_2009 <- as.integer(trend_pred_data$year >= 2007 & trend_pred_data$year <= 2010)
trend_pred_data$shock_2020 <- as.integer(trend_pred_data$year >= 2019 & trend_pred_data$year <= 2021)

# Identify seasonal smooth terms to exclude
smooth_labels <- sapply(model$smooth, function(x) x$label)
seasonal_smooths <- grep("s\\(month", smooth_labels, value = TRUE)

# Predict excluding both seasonal components: shared s(month) and by=education s(month)
trend_preds <- predict(model,
                      newdata = trend_pred_data,
                      type = "response",
                      se.fit = TRUE,
                      exclude = seasonal_smooths)
...
seasonal_data <- expand_grid(
  ...
  time_index = 200,  # Mid-series (2016, no shock)
  month = 1:12,
  shock_2008_2009 = 0,
  shock_2020 = 0
) %>%
...
# Extract just the shared seasonal smooth term s(month)
# Using terms parameter to isolate only the month smooth contribution
shared_seasonal_data <- expand.grid(
  month = 1:12,
  time_index = 200,
  education = "phd",  # Education doesn't affect the shared smooth
  shock_2008_2009 = 0,  # Time index 200 is in 2016, no shock
  shock_2020 = 0
) %>%
  as.data.frame()

# Extract the contribution of just s(month) smooth (shared component only)
# by using terms = "s(month)" to isolate this smooth's partial effect
shared_smooth_preds <- predict(model,
                               newdata = shared_seasonal_data,
                               type = "link",  # Get linear predictor contribution
                               terms = "s(month)",  # Only the shared month smooth
                               se.fit = TRUE)

# Extract point estimates and standard errors
shared_smooth_effect <- shared_smooth_preds$fit
shared_smooth_se <- shared_smooth_preds$se.fit

# Create plot data with uncertainty
shared_seasonal_plot <- data.frame(
  month = 1:12,
  smooth_effect = shared_smooth_effect,
  se = shared_smooth_se
)

# Center the effect for interpretability
shared_seasonal_plot$centered_effect <- shared_seasonal_plot$smooth_effect - mean(shared_seasonal_plot$smooth_effect)
shared_seasonal_plot$se_centered <- shared_seasonal_plot$se  # SE doesn't change with centering
shared_seasonal_plot$ci_lower <- shared_seasonal_plot$centered_effect - 1.96 * shared_seasonal_plot$se_centered
shared_seasonal_plot$ci_upper <- shared_seasonal_plot$centered_effect + 1.96 * shared_seasonal_plot$se_centered

# Plot the shared seasonal smooth pattern with uncertainty
ggplot(shared_seasonal_plot, aes(x = month, y = centered_effect)) +
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper),
              alpha = 0.2, fill = "#2E86AB", color = NA) +
  ...
  labs(
    ...
    subtitle = "Pure shared smooth s(month) extracted from model - the seasonal effect applied equally to all education levels",
    x = "Month",
    y = "Seasonal Effect (centered on log-odds scale)"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
...
```{r fig.height=10, fig.width=14}
# Extract education-specific deviations from the shared pattern
# by comparing total prediction to shared smooth only
...
# Get total prediction (all terms) for all education levels at each month WITH SEs
total_preds <- predict(model, newdata = all_education_seasonal, type = "link", se.fit = TRUE)
total_pred <- total_preds$fit
total_se <- total_preds$se.fit

# Get prediction with only shared smooth s(month) - same for all education levels
# Create dummy data just for shared smooth extraction (education doesn't matter)
shared_data_dummy <- expand_grid(
  month = 1:12,
  time_index = 200,
  education = "phd",
  shock_2008_2009 = 0,  # Time index 200 is in 2016, no shock
...
shared_preds_dummy <- predict(model, newdata = shared_data_dummy, type = "link",
                              terms = "s(month)", se.fit = TRUE)

# Extract shared predictions and SEs
shared_pred_dummy <- shared_preds_dummy$fit
shared_se_dummy <- shared_preds_dummy$se.fit

# Replicate shared predictions and SEs to match all education levels
n_ed <- length(unique(counts_data$education))
shared_pred <- rep(shared_pred_dummy, times = n_ed)
shared_se <- rep(shared_se_dummy, times = n_ed)

# Calculate education-specific deviation (total - shared)
# SEs propagate: SE(A-B) = sqrt(SE(A)^2 + SE(B)^2) assuming independence
education_deviation <- total_pred - shared_pred
deviation_se <- sqrt(total_se^2 + shared_se^2)

# Build data frame with deviations and uncertainties
deviation_data <- data.frame(
  education = all_education_seasonal$education,
  month = all_education_seasonal$month,
  shared_effect = shared_pred,
  total_effect = total_pred,
  education_deviation = education_deviation,
  deviation_se = deviation_se,
  ci_lower = education_deviation - 1.96 * deviation_se,
  ci_upper = education_deviation + 1.96 * deviation_se
)

# Plot education-specific deviations with confidence ribbons
ggplot(deviation_data, aes(x = month, y = education_deviation, color = education, fill = education)) +
  geom_ribbon(aes(ymin = ci_lower, ymax = ci_upper),
              alpha = 0.15, color = NA) +
  ...
  labs(
    ...
    x = "Month",
    y = "Deviation from Shared Pattern (log-odds scale)"
  ) +
...
This section visualizes each component of the GAM separately using marginal effects plots.
These plots isolate the contribution of each factor (education, shocks, seasonality) while
holding others at reference levels.

### Marginal Effects Panel

```{r fig.height=12, fig.width=14}
# Create comprehensive marginal effects panel
marginal_effects_plot <- plot_marginal_effects(model_result_quasi)
...
```{r}
edu_effects <- extract_education_effects(model_result_quasi)
cat("\n=== EDUCATION MAIN EFFECTS (Log-odds Scale) ===\n\n")
print(edu_effects)
cat("\nNote: Positive effects = higher unemployment risk\n")
cat("      Negative effects = lower unemployment risk\n")
```
