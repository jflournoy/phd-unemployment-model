...
$$s(\text{month}, k=14, \text{bs="cc"}) + s(\text{month}, k=14, \text{bs="cc"}, \text{by=education})$$
...
- **s(month, bs="cc")**: Shared cyclic cubic spline for seasonal patterns (k=14) applied to all education levels
- **s(month, bs="cc", by=education)**: Education-specific seasonal deviations (k=14) from the shared seasonal pattern
...
```{r}
# Fit quasi-binomial GAM (optimized with bam() for performance)
...
The quasi-binomial dispersion parameter indicates overdispersion in the data. Since our data is population-representative (not a sample), we test whether increasing the basis dimension (k) of the time smooth allows the model to capture more real variation, which would reduce the estimated dispersion.
