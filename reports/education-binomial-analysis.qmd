...
$$s(\text{time\_index}, k=150, \text{by=education}, \text{bs="cr"}) +$$
$$s(\text{time\_index}, k=10, \text{by=shock\_2008\_2009}, \text{bs="cr"}) +$$
$$s(\text{time\_index}, k=10, \text{by=shock\_2020}, \text{bs="cr"}) +$$
...
- **shock_2008_2009, shock_2020**: Binary indicators for major economic disruptions (2008-2009 financial crisis, 2020 COVID-19)
- **s(time_index, by=education)**: Education-specific smooth trends (k=150) capturing long-term unemployment dynamics
- **s(time_index, by=shock_*)**: Shock-specific time dynamics (k=10) allowing different crisis trajectories during disruptions
- **s(month, bs="cc")**: Shared cyclic cubic spline for seasonal patterns applied to all education levels
- **s(month, bs="cc", by=education)**: Education-specific seasonal deviations from the shared seasonal pattern
...
pred_data_comparison <- data.frame(
  ...
  time_index = 200,
  month = 6,
  shock_2008_2009 = 0,  # Time index 200 is June 2016, no shock
  shock_2020 = 0,
...
# Create prediction data for TRULY seasonality-free trend comparison
# Exclude seasonal smooth terms to isolate trends from seasonality
...
# Add shock variables based on year
trend_comparison_data$year <- 2000 + floor((trend_comparison_data$time_index - 1) / 12)
trend_comparison_data$shock_2008_2009 <- as.integer(trend_comparison_data$year >= 2008 & trend_comparison_data$year <= 2009)
trend_comparison_data$shock_2020 <- as.integer(trend_comparison_data$year == 2020)

# Identify seasonal smooth terms to exclude
smooth_labels <- sapply(model_result_quasi$model$smooth, function(x) x$label)
seasonal_smooths <- grep("s\\(month", smooth_labels, value = TRUE)

# Get predictions from both models, excluding seasonal components
...
trend_pred_data$shock_2008_2009 <- as.integer(trend_pred_data$year >= 2008 & trend_pred_data$year <= 2009)
trend_pred_data$shock_2020 <- as.integer(trend_pred_data$year == 2020)
