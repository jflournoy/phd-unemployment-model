...
shared_smooth_effect <- predict(model,
                                newdata = shared_seasonal_data,
                                type = "link",  # Get linear predictor contribution
                                terms = "s(month)",  # Only the shared month smooth
                                se.fit = FALSE)

# Convert from link scale to percentage point changes
# The shared smooth contributes as additive effect on log-odds
# We'll show the relative effect pattern
...
# Plot the shared seasonal smooth pattern
...
# Get total prediction (all terms) for all education levels at each month
total_pred <- predict(model, newdata = all_education_seasonal, type = "link", se.fit = FALSE)
...
shared_pred_dummy <- predict(model, newdata = shared_data_dummy, type = "link",
                             terms = "s(month)", se.fit = FALSE)

# Replicate shared predictions to match all education levels
shared_pred <- rep(shared_pred_dummy, times = length(unique(counts_data$education)))
